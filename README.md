...
| `docker-compose.yml` | Оркестрація Docker контейнерів (Redis, Nginx, ML) |
...
http {
    ...
    upstream backend {
        server host.docker.internal:8000;  # ML сервер на Python (FastAPI)
    }
    ...
    server {
        ...
        # Проксі для API запитів до ML сервісу
        location /api/ {
...
Пояснення проксі-налаштувань
Директива	Значення	Пояснення
proxy_http_version 1.1	HTTP/1.1	Встановлює версію протоколу для зв'язку з backend-сервером. HTTP/1.1 підтримує keep-alive з'єднання, що зменшує затримки при частих запитах.
proxy_set_header Host $host	Оригінальний хост	Передає на backend оригінальне значення заголовка Host від клієнта.
proxy_set_header X-Real-IP $remote_addr	Реальна IP-адреса клієнта	Передає справжню IP-адресу клієнта. Без цього backend бачив би лише IP проксі. Критично для системи безпеки.
proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for	Ланцюжок проксі	Додає IP поточного проксі до списку в X-Forwarded-For. Дозволяє відстежувати весь ланцюжок проксі.
proxy_set_header X-Forwarded-Proto $scheme	Оригінальний протокол	Передає інформацію про те, чи прийшов запит по HTTP або HTTPS.
...
Додати у реальний Nginx конфіг необхідний log_format (щоб система правильно парсила логи)

Вказати у config.json реальний шлях до логів (наприклад, /var/log/nginx/access.log)
...
Встановіть Docker Desktop for Windows.
